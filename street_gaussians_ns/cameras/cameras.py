...
class Cameras(TensorDataclass):
    ...
    camera_ids: Optional[Int[Tensor, "*num_cameras 1"]]
    ...
    def __init__(
        ...
        camera_ids: Optional[Int[Tensor, "*num_cameras 1"]] = None,
        ...
        self.camera_ids = self._init_camera_ids(camera_ids)
    ...
    def _init_camera_ids(self, camera_ids: Int[Tensor, "*num_cameras 1"]) -> torch.Tensor:
        """
        Parses the input camera_ids and returns a tensor of the correct shape
        Only needs to make sure that we a 1 in the last dimension if it is a tensor.
        Args:
            camera_ids: The focal length / principle point x or y
            name: The name of the variable. Used for error messages
        """
        if camera_ids is None:
            camera_ids = torch.ones_like(self.fx, dtype=torch.uint8)
        elif isinstance(camera_ids, torch.Tensor):
            if camera_ids.ndim == 0 or camera_ids.shape[-1] != 1:
                camera_ids = camera_ids.unsqueeze(-1)
            camera_ids = camera_ids.to(self.device)
        else:
            raise ValueError(f"camera_ids must be a float or tensor, got {type(camera_ids)}")
        return camera_ids
    ...
    def rescale_output_resolution(
        ...
        self.height = (self.height * scaling_factor).ceil().to(torch.int64)
        self.width = (self.width * scaling_factor).ceil().to(torch.int64)
